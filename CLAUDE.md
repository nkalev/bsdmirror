...
- **Tooling runs, in CI and in a container.** `pyproject.toml` configures ruff, pytest, coverage and bandit. `.github/workflows/ci.yml` has 11 jobs with `ci` as the required aggregate. `tests/` holds the full suite, plus two Node harnesses in `tests/js/` that drive real headless Chrome — its size is not restated here as a count; that line has gone stale twice already, so trust `docker compose run --rm test`'s own summary line instead of a number pinned in this file. Run it there, never on the host, and read the **exit code**, not the last line: piping the run through `tail` reports tail's status and turns a red suite green.
