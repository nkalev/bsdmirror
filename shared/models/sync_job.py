...
class SyncJob(Base):
    ...
    mirror_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("mirrors.id", ondelete="CASCADE"), nullable=False, index=True
    )
    status: Mapped[SyncStatus] = mapped_column(
        SQLEnum(SyncStatus, name="sync_status"), default=SyncStatus.PENDING, nullable=False
    )
    ...
    triggered_by: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, comment="scheduled, manual, or username"
    )
