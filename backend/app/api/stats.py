...
@router.get("/overview")
async def get_stats_overview(db: AsyncSession = Depends(get_db)) -> dict:
    ...
    return {
        "mirrors": {
            name: {
                ...
                "last_updated": m.last_sync_completed.isoformat()
                if m.last_sync_completed
                else None,
                "size": humanize.naturalsize(m.total_size_bytes)
...
async def get_sync_activity(days: int = 7, db: AsyncSession = Depends(get_db)) -> dict:
    """Get sync activity for the last N days."""
    cutoff = datetime.now(timezone.utc) - timedelta(days=days)

    result = await db.execute(
        select(SyncJob).where(SyncJob.created_at >= cutoff).order_by(SyncJob.created_at.desc())
    )
        ...
        "recent_syncs": [
            {
                ...
                "bytes_transferred": humanize.naturalsize(job.bytes_transferred)
