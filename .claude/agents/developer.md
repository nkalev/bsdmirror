...
- **Models live in `shared/models/`**, imported by both `backend/app/` and `sync/sync_service.py`. A model change now lands once. There are still no migrations, so any column or type change needs a deliberate plan — `create_all` will not alter an existing table.
